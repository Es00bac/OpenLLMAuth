...
Use the package under `src/open_llm_auth`. Ignore duplicate legacy trees such as `src/src/open_llm_auth` and packaged artifacts under `pkg/` when reasoning about current behavior.
...
- `src/open_llm_auth/server/auth.py`
- `src/open_llm_auth/server/routes.py`
