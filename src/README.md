...
The active package is [`open_llm_auth`](./open_llm_auth). Duplicate trees such as `src/src/open_llm_auth` and packaged artifacts under `pkg/` are not the live source of truth for current development.

## Current Architecture

Live entrypoints:
- [`open_llm_auth/main.py`](./open_llm_auth/main.py): FastAPI app, static UI mount, root/chat/config pages
- [`open_llm_auth/server/routes.py`](./open_llm_auth/server/routes.py): `/v1/*` API routes
- [`open_llm_auth/server/config_routes.py`](./open_llm_auth/server/config_routes.py): `/config/*` admin/config routes
- [`open_llm_auth/cli.py`](./open_llm_auth/cli.py): Typer CLI
...
Current auth behavior from `open_llm_auth/server/auth.py`:
...
Current behavior from `open_llm_auth/providers/agent_bridge.py`:
